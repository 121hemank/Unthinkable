...
export async function searchDoctors(req: Request, res: Response, next: NextFunction) {
    try {
        const { specialization } = req.query as {
...
const STANDARD_SLOTS = ["08:00", "14:00", "20:00"];
function frequencyToTimes(frequency: string): string[] {
    ...
    if (dash) {
        ...
        const times: string[] = [];
        if (morning)
            times.push(STANDARD_SLOTS[0]);
        if (afternoon)
            times.push(STANDARD_SLOTS[1]);
        if (night)
            times.push(STANDARD_SLOTS[2]);
        return times.length ? times : [STANDARD_SLOTS[0]];
    }
    let count = 1;
    if (/three|thrice|tds|tid|\b3\s*(x|times)?\b/.test(f))
        count = 3;
    else if (/four|qid|\b4\s*(x|times)?\b/.test(f))
        count = 4;
    else if (/two|twice|double|\b2\s*(x|times)?\b/.test(f))
        count = 2;
    ...
    if (count === 1) {
        if (wantsNight && !wantsMorning)
            return [STANDARD_SLOTS[2]];
        if (wantsAfternoon)
...
export async function upsertMyProfile(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (slotDurationMinutes)
            update.slotDurationMinutes = slotDurationMinutes;
        if (workingHours) {
            update.workingHours = {
                mon: [],
                tue: [],
        ...
        const profile = await DoctorProfile.findOneAndUpdate({ userId: req.user!.userId }, { $setOnInsert: { userId: req.user!.userId }, $set: update }, { upsert: true, new: true, runValidators: true });
        return res.json({ profile });
    }
    catch (err) {
        next(err);
