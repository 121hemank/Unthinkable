...
export async function generatePostVisitSummary(clinicalNotes: string, prescription: {
    medicationName: string;
    dosage: string;
    frequency: string;
    durationDays: number;
    ...
    const rxList = prescription.length
        ? prescription
            .map((p, i) => `${i + 1}. ${p.medicationName} — ${p.dosage} — ${p.frequency} — ${p.durationDays} day(s)`)
            .join("\n")
