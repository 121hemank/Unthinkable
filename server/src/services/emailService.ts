...
function getTransporter(): nodemailer.Transporter {
  ...
  transporter = nodemailer.createTransport({
    ...
    // implicit TLS on 465 (Gmail), STARTTLS on 587 (Ethereal, Mailtrap, ...)
...
/**
 * Sends an email. NEVER throws — callers (notificationService) handle
 * success/failure by checking the return value and updating NotificationLog
 * accordingly. This keeps a broken SMTP config from ever breaking a
 * booking/cancellation flow. See §3.4 of the design doc.
 */
...
// Clinic timezone for every human-readable time in emails
...
export const emailTemplates = {
  ...
  // The doctor's copy must not address their own patient as "Dr."
