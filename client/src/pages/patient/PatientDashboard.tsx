...
interface VisitRecord {
  symptomForm: { rawSymptoms: string } | null;
  postVisitNote: PostVisitNoteT | null;
  postVisitSummary: PostVisitSummaryT | null;
}

/**
 * Expanded view for a completed appointment: AI patient-friendly summary
 * (with graceful fallback to raw notes when llmStatus is FAILED), the
 * medication schedule, and the prescription as prescribed.
 */
function VisitSummaryPanel({ apptId }: { apptId: string }) {
  ...
  if (error) return <p className="text-sm text-urgency-high">Could not load visit summary.</p>;
  if (!record) return <p className="text-sm text-slate-500">Loading…</p>;
  ...
  return (
    <div className="mt-4 space-y-5">
      {/* AI summary — or honest fallback when the LLM failed */}
      {s?.llmStatus === "OK" && s.patientFriendlyText ? (
      ...
      {/* Medication schedule */}
      {s?.llmStatus === "OK" && s.medicationSchedule.length > 0 && (
        <div>
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {s.medicationSchedule.map((m, i) => (
              <div
                key={i}
                className="flex items-center justify-between gap-3 bg-white border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm"
              >
      ...
      {/* The symptoms the patient themselves submitted */}
      {record.symptomForm && (
        <div>
...
/** Three-dot progress indicator across the booking flow. */
    ...
    <div className="flex items-center mb-6">
      {STEPS.map((s, i) => (
        <Fragment key={s.label}>
          {i > 0 && (
            <div className={`h-0.5 flex-1 mx-3 ${i <= current ? "bg-primary" : "bg-slate-200"}`} />
          )}
...
/** How far ahead patients may book — spans multiple months. */
...
/** Compact weekday chips for quick selection of the coming week. */
function DayChips({ selected, onPick }: { selected: string; onPick: (d: string) => void }) {
  return (
    ...
    <div className={`grid ${cols} gap-2`}>
      {slots.map((s) => (
        <button
          ...
          disabled={busy}
          onClick={() => onPick(s)}
          className="border border-slate-300 rounded-lg py-2 text-sm font-medium text-slate-700 transition-colors
...
/** Live mm:ss countdown until the HELD appointment's holdExpiresAt. */
function HoldCountdown({ expiresAt, onExpired }: { expiresAt: string; onExpired: () => void }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-mono ${
        remaining < 60000 ? "bg-red-50 text-urgency-high" : "bg-amber-50 text-amber-700"
      }`}
    >
...
/** Expandable appointment card for the patient's upcoming visits. */
  ...
  return (
    <div
      className={`border border-slate-200 rounded-xl bg-white overflow-hidden ${
        variant === "held" ? "border-l-4 border-l-amber-400" : ""
      }`}
    >
...
export default function PatientDashboard() {
  ...
  const loadAppointments = useCallback(async () => {
    try {
      ...
      /* dashboard list is non-critical */
    }
  }, []);
  ...
  // Load all doctors immediately so the list isn't empty on arrival
  useEffect(() => {
    api
      .get("/doctors")
  ...
  useEffect(() => {
    if (step === "slots" && doctor) loadSlots(doctor._id, date);
  ...
  async function pickSlot(slot: Slot) {
    ...
    try {
      ...
      loadSlots(doctor._id, date); // refresh grid after losing a race
    } finally {
      setBusy(false);
    }
  }
  ...
  async function confirm() {
    ...
    try {
      ...
      if (err?.response?.status === 410) {
        // hold expired server-side — restart the flow
        setHeld(null);
        setStep("slots");
      }
    } finally {
  ...
  function releaseHold() {
    if (held) api.post(`/appointments/${held._id}/cancel`).catch(() => {});
    setHeld(null);
  ...
  /**
   * Resume an abandoned hold from the dashboard list. If it already expired,
   * release it server-side and tell the patient the slot is free again.
   */
  function resumeHold(a: Appointment) {
    ...
    // Came from the dashboard, not the search flow — no doctor context yet.
    setDoctor(null);
    setHeld(a);
    setSymptoms("");
    setSymptomsSaved(false);
  ...
  /* ---------------- Derived views over my appointments ---------------- */
  // Ticks every 30s so expired holds drop out of the list on their own.
  ...
  const nextAppt = [...groups.upcoming].sort(
    ...
    >
      ...
      <Card title="Book an appointment">
        ...
        {step === "search" && (
          <>
            <div className="flex gap-2 mb-5">
              <input
                className={inputClass}
                placeholder="Search by specialization (e.g. Cardiology) or leave blank for all"
            ...
            {doctors.length === 0 ? (
              <p className="text-sm text-slate-500">No doctors to show yet — run a search above.</p>
            ) : (
              <ul className="divide-y divide-slate-100 -mx-2">
                {doctors.map((d) => (
                  <li key={d._id} className="py-3 px-2 flex items-center gap-4">
        ...
        {step === "slots" && doctor && (
          <>
            ...
            {loadingSlots ? (
              ...
              <SlotGrid slots={slots} busy={busy} onPick={pickSlot} />
            )}
          </>
        )}
        ...
        {step === "booking" && held && (
          <>
            ...
            {!symptomsSaved && (
              <p className="text-xs text-slate-400 mt-2">Save your symptoms first, then confirm.</p>
            )}
          </>
        )}
      </Card>
      ...
      <Card
        ...
        actions={
          groups.upcoming.length > 0 ? (
            <span className="rounded-full bg-primary-light text-primary-dark px-2.5 py-0.5 text-xs font-medium">
          ...
          <div className="space-y-3">
            {groups.upcoming.map((a) => (
              <UpcomingRow
                key={a._id}
                appt={a}
                variant={a.status === "HELD" ? "held" : "confirmed"}
                onReschedule={() => startReschedule(a)}
                onResume={() => resumeHold(a)}
                onRelease={() => cancelAppointment(a._id)}
      ...
      <Card title="Visit history & summaries">
        {groups.completed.length === 0 && groups.cancelled.length === 0 ? (
          <p className="text-sm text-slate-500">Completed and cancelled visits will be listed here.</p>
        ) : (
          <ul className="divide-y divide-slate-100 -mx-2">
            {[...groups.completed, ...groups.cancelled].map((a) => (
              <li key={a._id}>
                <div className="py-3 px-2 flex items-center gap-4">
                  ...
                  {a.status === "COMPLETED" && (
                    <button
                      onClick={() => setExpanded(a._id === expanded ? null : a._id)}
                      className="text-sm text-primary font-medium hover:underline whitespace-nowrap"
                    >
        ...
        {rescheduling && (
          <div className="mt-5 border-t border-slate-200 pt-4">
            ...
            {loadingSlots ? (
              ...
              <SlotGrid slots={rsSlots} busy={busy} cols="grid-cols-4 sm:grid-cols-6" onPick={pickRescheduleSlot} />
            )}
          </div>
        )}
      </Card>
