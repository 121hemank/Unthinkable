...
/** Leave date ("YYYY-MM-DD") shown as a friendly weekday label. */
function leaveDateLabel(date: string): string {
  const d = new Date(`${date}T00:00:00`);
  const today = new Date();
  const dayNumber = (x: Date) => Math.floor(Date.UTC(x.getFullYear(), x.getMonth(), x.getDate()) / 86_400_000);
...
/**
 * Working hours are entered as compact strings per weekday, e.g.
 *   mon: "09:00-13:00, 14:00-17:00"
 * which we parse into the [{start,end}] shape the API expects.
 */
function parseHours(input: string): { start: string; end: string }[] {
  return input
    .split(",")
...
function hoursToText(hours: Record<string, { start: string; end: string }[]> | undefined): string {
  if (!hours) return "";
...
/** Weekly schedule strip shown when a roster row is expanded. */
function WeeklySchedule({ doctor }: { doctor: DoctorListItem }) {
  const hours = doctor.profile?.workingHours;
  if (!hours) return null;
  return (
    <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 mt-3 mb-1">
      {WEEKDAYS.map((d) => {
        ...
        return (
          <div
            ...
            className={`rounded-lg border px-2.5 py-2 ${
              off ? "border-slate-100 bg-slate-50" : "border-primary/20 bg-primary-light/30"
            }`}
          >
            ...
            {off ? (
              <p className="text-[11px] text-slate-400 mt-0.5">Day off</p>
            ) : (
              <div className="mt-0.5 space-y-0.5">
                {windows.map((w, i) => (
                  <p key={i} className="text-[11px] text-slate-600 font-mono leading-tight">
                    {w.start}–{w.end}
                  </p>
                ))}
              </div>
...
export default function AdminDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (apptRes.status === "fulfilled") setAllAppts(apptRes.value.data.appointments);
  ...
  async function createProfile() {
    ...
    setMessage(null);
    try {
      // parse "mon: 09:00-13:00" lines -> workingHours object
      ...
      for (const line of hoursText.split("\n")) {
        ...
        if (!WEEKDAYS.includes(dayPart as WeekdayKey)) continue;
        // rejoin the remainder so "09:00" isn't split on its own colon
  ...
  /** Flip a doctor's profile between accepting bookings and paused. */
  ...
  /** Administrative cancellation with notifications to both parties. */
  ...
  /** Withdraw a future leave day so the doctor becomes bookable again. */
  ...
  const filteredAppts =
    apptFilter === "ALL" ? allAppts : allAppts.filter((a) => a.status === apptFilter);

  const filteredUsers =
    userFilter === "ALL" ? users : users.filter((u) => u.role === userFilter);

  /** Enable/disable any non-admin account; takes effect immediately. */
  ...
  /**
   * Contextual leave flow: clicking a doctor's "Mark leave" pre-selects them
   * and brings the form into view, instead of making the admin pick from a
   * dropdown detached from the roster.
   */
  ...
  return (
    <DashboardShell title="Admin console" subtitle="Manage doctors, profiles and leave days">
      {message && (
        <div
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Stat
          ...
          accent={activeProfiles > 0 ? "border-emerald-500" : "border-slate-300"}
        />
        ...
        <Stat
          ...
          accent={(stats?.byStatus?.CANCELLED ?? 0) > 0 ? "border-red-400" : "border-slate-300"}
        />
      </div>
              ...
              <Fragment key={d._id}>
                <li className="py-3 px-2 flex items-center gap-4">
                  ...
                  {d.profile ? (
                    <>
                      {d.profile.isActive ? (
                        <span className="hidden sm:inline-block rounded-full bg-primary-light text-primary-dark px-2.5 py-0.5 text-xs font-medium whitespace-nowrap">
                          {d.profile.specialization}
                        </span>
                      ) : (
                        <span className="rounded-full bg-slate-200 text-slate-600 px-2.5 py-0.5 text-xs font-medium whitespace-nowrap">
                          Paused
                        </span>
                      )}
                      ...
                      <button
                        ...
                        className={`text-sm font-medium hover:underline whitespace-nowrap ${
                          d.profile.isActive ? "text-urgency-medium" : "text-emerald-600"
                        }`}
                      >
                        {d.profile.isActive ? "Deactivate" : "Activate"}
                      </button>
                      <button
                        onClick={() => startLeaveFor(d)}
                        className="text-sm text-amber-600 font-medium hover:underline whitespace-nowrap"
                      >
                        Mark leave
                      </button>
                      <button
        ...
        {unprofiled.length === 0 ? (
          ...
          <div className="space-y-4 max-w-xl">
            ...
            <button
              ...
              disabled={busy || !selectedUserId || specialization.trim().length < 2}
              className={btnClass}
            >
              Create profile
            </button>
      ...
      <Card
        ...
        actions={
          <select
            className="text-sm border border-slate-300 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-primary"
            value={userFilter}
      ...
      >
        {filteredUsers.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-100 -mx-2">
            {filteredUsers.map((u) => (
              <li key={u._id} className="py-3 px-2 flex items-center gap-4">
                ...
                <span
                  className={`hidden sm:inline-block rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${
                    u.role === "patient"
                ...
                {u.isActive ? (
                  <span className="inline-flex items-center gap-1.5 text-xs text-emerald-700 whitespace-nowrap">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Active
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 text-xs text-red-600 whitespace-nowrap">
                ...
                {u.role !== "admin" && (
                  <button
                    ...
                    disabled={busy}
                    className={`text-sm font-medium hover:underline whitespace-nowrap ${
                      u.isActive ? "text-urgency-high" : "text-emerald-600"
                    }`}
                  >
      ...
      <Card title="Upcoming leave days">
        {leaves.length === 0 ? (
          <p className="text-sm text-slate-500">No upcoming leave days on the calendar.</p>
        ) : (
          <ul className="divide-y divide-slate-100 -mx-2">
            {leaves.map((l) => (
              <li key={l._id} className="py-3 px-2 flex items-center gap-4">
      ...
      >
        {filteredAppts.length === 0 ? (
          <p className="text-sm text-slate-500">No appointments match this filter yet.</p>
        ) : (
          <ul className="divide-y divide-slate-100 -mx-2">
            {filteredAppts.map((a) => (
              <li key={a._id} className="py-3 px-2 flex items-center gap-4">
                ...
                {a.status === "CONFIRMED" && (
                  <button
                    onClick={() => adminCancel(a._id)}
                    disabled={busy}
                    className="text-sm text-urgency-high hover:underline whitespace-nowrap"
                  >
                    Cancel
                  </button>
                )}
