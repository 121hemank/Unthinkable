...
/**
 * Patient's past clinic visits (completed/cancelled) with prescriptions —
 * loaded lazily when a row is expanded. The server only serves history for
 * patients the doctor actually shares appointments with.
 */
function PatientHistory({ patientId }: { patientId: string }) {
  ...
  return (
    <div className="bg-slate-50 rounded-lg p-4">
      ...
      {error ? (
        <p className="text-sm text-urgency-medium">Could not load patient history.</p>
      ) : visits === null ? (
        <p className="text-sm text-slate-500">Loading…</p>
      ) : visits.length === 0 ? (
        <p className="text-sm text-slate-500">First visit — no prior clinic history.</p>
      ) : (
        <ul className="space-y-2">
          {visits.map((v) => {
            const meds = v.prescription.map((p) => p.medicationName).join(", ");
            return (
              <li key={v._id} className="flex items-center gap-3 text-sm">
                <span className="text-slate-700 font-medium whitespace-nowrap">{whenLabel(v.slotStart)}</span>
                <span
                  className={`text-xs whitespace-nowrap ${
                    v.status === "COMPLETED" ? "text-emerald-700" : "text-slate-400"
                  }`}
                >
...
/**
 * Expandable appointment row. The pre-visit brief is always available;
 * the post-visit form only renders for visits whose time has arrived
 * (the server enforces this too — see submitPostVisit).
 */
function AppointmentRow({
  ...
  const [brief, setBrief] = useState<{ symptomForm: SymptomForm | null; summary: PreVisitSummary | null } | null>(null);
  ...
  return (
    <div className={`${accent} border border-slate-200 rounded-xl bg-white transition-colors`}>
      <button onClick={toggle} className="w-full text-left px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          {variant === "attention" && (
            <span className="hidden sm:inline-block rounded-full bg-amber-100 text-amber-800 px-2.5 py-0.5 text-xs font-medium">
              Awaiting notes
            </span>
          )}
      ...
      {open && (
        <div className="px-5 pb-5 space-y-5 border-t border-slate-100 pt-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                {brief?.summary?.llmStatus === "OK" && brief.summary.urgencyLevel && (
                  <UrgencyBadge level={brief.summary.urgencyLevel} />
                )}
              </div>
              {brief?.summary?.llmStatus === "OK" ? (
                <>
                  ...
                  <ul className="list-disc list-inside text-sm text-slate-700">
                    {brief.summary.suggestedQuestions.map((q, i) => (
                      <li key={i}>{q}</li>
                    ))}
                  </ul>
                </>
          ...
          {/* Past clinic visits — consultation context */}
          {typeof appt.patientId !== "string" && (
            <PatientHistory patientId={appt.patientId._id} />
          )}

          {/* Post-visit form — only when the visit time has arrived */}
          {variant === "attention" ? (
            <>
              <div>
              ...
              <div>
                ...
                {rx.map((item, i) => (
                  <div key={i} className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-2">
                    ...
                    <input
                      className={inputClass}
                      placeholder='Frequency ("1-0-1" or words)'
                    ...
                    />
                    <button
                      onClick={() => setRx((p) => p.filter((_, idx) => idx !== i))}
                      className="text-sm text-slate-500 hover:text-urgency-high"
                    >
                      Remove
                    </button>
                  </div>
...
export default function DoctorDashboard() {
  ...
  // Re-group every minute so "upcoming" flows into "awaiting notes" on its own
  useEffect(() => {
  ...
  return (
    <DashboardShell
      title="Dr. dashboard"
      subtitle="Your appointments, pre-visit briefs and post-visit notes"
    >
      {/* Overview */}
      ...
      >
        {!loaded ? (
          ...
          <div className="space-y-3">
            {groups.attention.map((a) => (
              <AppointmentRow key={a._id} appt={a} variant="attention" onSubmitted={load} />
            ))}
          </div>
        )}
      </Card>
        ...
        {!loaded ? (
          ...
          <div className="space-y-3">
            {groups.upcoming.map((a) => (
              <AppointmentRow key={a._id} appt={a} variant="upcoming" onSubmitted={load} />
            ))}
          </div>
        )}
      </Card>
      ...
      <Card title="Recent history">
        {groups.completed.length === 0 ? (
          <p className="text-sm text-slate-500">Completed visits will be listed here.</p>
        ) : (
          <ul className="divide-y divide-slate-100">
            {groups.completed.slice(0, 6).map((a) => (
