...
export function UrgencyBadge({ level }: { level: string }) {
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${
        URGENCY_STYLES[level] || "bg-slate-200 text-slate-600"
      }`}
    >
      {level} urgency
    </span>
...
/** Shared input styles so every form looks identical */
export const inputClass =
  "w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary";
export const btnClass =
  "bg-primary hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed text-white py-2 px-4 rounded-lg font-medium";

/** Overview stat tile used at the top of both dashboards. */
export function Stat({ label, value, accent }: { label: string; value: number; accent: string }) {
  return (
    <div className={`bg-white border border-slate-200 border-l-4 ${accent} rounded-xl px-5 py-4`}>
  ...
  return (
    <div
      className={`shrink-0 rounded-full flex items-center justify-center font-semibold ${
        tones[tone]
      } ${size === "sm" ? "w-9 h-9 text-xs" : "w-10 h-10 text-sm"}`}
    >
      {initials(name)}
    </div>
  );
